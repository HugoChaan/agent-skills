...
| Skill | 产品 | 说明 |
|-------|------|------|
| [integrate-shengwang-conversational-ai](skills/integrate-shengwang-conversational-ai/SKILL.md) | ConvoAI | AI 语音 agent 全流程：创建/停止/更新/查询，支持 Go/Java/Python |
| [integrate-shengwang-rtc](skills/integrate-shengwang-rtc/SKILL.md) | RTC SDK | 实时音视频通话，支持 Web/Android/iOS/Flutter 等 |
| [integrate-shengwang-rtm](skills/integrate-shengwang-rtm/SKILL.md) | RTM | 实时消息、信令、Presence |
| [integrate-shengwang-cloud-recording](skills/integrate-shengwang-cloud-recording/SKILL.md) | Cloud Recording | 服务端录制 RTC 会话 |
| [implement-shengwang-token-on-server](skills/implement-shengwang-token-on-server/SKILL.md) | Token Server | 服务端 Token 生成（AccessToken2） |
| [general](skills/general/SKILL.md) | 通用 | 凭证管理、REST 认证模式 |
| [resource-downloader](skills/resource-downloader/SKILL.md) | 工具 | 下载 SDK、示例项目、Token Builder |
| [intake](skills/intake/SKILL.md) | 路由 | 需求分析 → 产品推荐 → 路由到具体 skill |

## 快速开始

### 1. 安装

将本仓库克隆或添加到你的 AI coding agent 的 skills 目录：
...
安装后，agent 检测到相关任务时会自动触发对应 skill（如 "做一个语音 AI"、"接入 Agora RTC"、"生成 token"）。
...
根路由 (`skills/SKILL.md`) 判断请求是否足够明确：
- 明确的 → 直接路由到对应产品 skill
